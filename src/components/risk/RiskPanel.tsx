    ...
    <div className="space-y-6">
      ...
      <GlassCard className="border-cyan-500/30">
        <div className="flex items-center gap-3 mb-5">
          ...
          <h2 className="text-lg font-bold text-gradient">Live Risk Summary</h2>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-800">
            <p className="text-xs text-slate-500 mb-1">Suggested Risk / Trade</p>
            <p className="text-xl font-bold text-cyan-400">{formatPercent(suggestedRiskPct)}</p>
          </div>
          <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-800">
            <p className="text-xs text-slate-500 mb-1">Suggested Safe Lot (100pt SL)</p>
            <p className="text-xl font-bold text-green-400">{safeLot.toFixed(2)}</p>
          </div>
          <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-800">
            <p className="text-xs text-slate-500 mb-1">Max Trades (5% Daily DD)</p>
            <p className="text-xl font-bold text-purple-400">{suggestedMaxTrades}</p>
          </div>
          <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-800">
            <p className="text-xs text-slate-500 mb-1">Safe Money at Risk</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <GlassCard>
          <div className="flex items-center gap-3 mb-5">
            ...
            <h2 className="text-lg font-bold text-white">Lot Size Calculator</h2>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2 mb-4">
              <button onClick={() => setRiskPct("0.5")} className="flex-1 py-1 text-[10px] uppercase rounded bg-slate-800 text-slate-400 hover:text-white">Conservative 0.5%</button>
              <button onClick={() => setRiskPct("1.0")} className="flex-1 py-1 text-[10px] uppercase rounded bg-slate-800 text-slate-400 hover:text-white">Balanced 1.0%</button>
              <button onClick={() => setRiskPct("2.0")} className="flex-1 py-1 text-[10px] uppercase rounded bg-slate-800 text-slate-400 hover:text-white">Aggressive 2.0%</button>
            </div>
            ...
            <div className="mt-6 p-4 rounded-xl bg-slate-900 border border-slate-800 text-center">
              <p className="text-slate-500 text-xs mb-1">Recommended Lot Size</p>
              ...
              <p className="text-sm text-slate-400 mt-2">
                Risking <strong className="text-pink-400">{formatCurrency(riskAmount)}</strong>
              </p>
            </div>
          </div>
        </GlassCard>
        ...
        <div className="space-y-6">
          <GlassCard>
            <div className="flex items-center gap-3 mb-5">
              ...
              <h2 className="text-lg font-bold text-white">Reverse Risk Calculator</h2>
            </div>
            ...
            <div className="space-y-4 pt-10">
              ...
              <div className="mt-6 p-4 rounded-xl bg-slate-900 border border-slate-800 text-center">
                <p className="text-slate-500 text-xs mb-1">Actual Risk Percentage</p>
                ...
                <p className="text-sm text-slate-400 mt-2">
                  Risking <strong className="text-pink-400">{formatCurrency(revRiskAmount)}</strong>
                </p>
                {revRiskPct > 3 && (
                  <p className="text-xs text-pink-500 mt-2 flex justify-center items-center gap-1">
                    <AlertTriangle className="h-3 w-3" /> High Risk Exposure
                  </p>
                )}
              </div>
            </div>
          ...
          <GlassCard>
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-base font-bold text-white">Weekly Compounding Preview</h2>
            </div>
              ...
              <table className="w-full text-sm text-left">
                <thead className="bg-slate-800/50 text-xs uppercase text-slate-400">
                  <tr>
                    ...
                    <th className="px-4 py-2 font-medium">Est. Balance</th>
                  ...
                  {[1, 2, 3, 4, 5].map(day => {
                    ...
                    return (
                      <tr key={day} className="hover:bg-slate-800/20">
                        <td className="px-4 py-3 text-slate-300">Day {day}</td>
            ...
            <p className="text-xs text-slate-500 mt-3 text-center">
              Assumes +1R per day hitting target continuously
            </p>
          </GlassCard>
        </div>
